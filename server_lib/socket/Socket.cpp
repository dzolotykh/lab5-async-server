...
Server::Socket::predicate_t Server::Socket::read_bytes_nonblock(size_t need_read, char *dst, size_t buff_size,
                                                                const Server::Socket::on_read_t &on_read) const {
    return NonblockingReader(need_read, dst, buff_size, on_read, *this);
}

Server::Socket::predicate_t
Server::Socket::read_bytes_nonblock(char *dst, size_t buff_size, const Server::Socket::on_read_t &on_read) const {
...
Server::Socket::NonblockingReader::NonblockingReader(size_t need_read, char *dst, size_t buff_size,
                                                     Server::Socket::on_read_t on_read, const Socket& _client)
    : need_read(need_read), dst(dst), buff_size(buff_size), on_read(std::move(on_read)), bytes_read(0), client(_client) {}
...
bool Server::Socket::NonblockingReader::operator()() {
    ...
    if (bytes_read < need_read) {
        ...
        if (read == -1 && errno != EAGAIN) {
            throw SocketException("Ошибка при чтении данных из сокета. Код ошибки: " + std::to_string(errno));
        } else if (read == -1) {
            return true;
        } else if (read == 0) {
...
Server::Socket::NonblockingReader::NonblockingReader(char *dst, size_t buff_size, Server::Socket::on_read_t on_read,
                                                     const Server::Socket &_client): dst(dst), buff_size(buff_size), on_read(std::move(on_read)), client(_client), read_all(true) {}
