...
class DigitsDrawer
{
  public:
    ...
    static void draw_digit(size_t x, size_t y, int digit, T_draw_pixel draw_pixel, size_t scale)
    {
        ...
        draw_from_file(
            x, y, "../resources/digits/" + std::to_string(digit) + ".txt", draw_pixel, scale);
    }

  private:
    template <typename T_draw_pixel>
    static void draw_from_file(
        size_t x, size_t y, const std::string& filepath, T_draw_pixel draw_pixel, size_t scale)
    {
