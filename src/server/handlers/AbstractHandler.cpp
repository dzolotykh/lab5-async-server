...
std::function<bool()> Server::AbstractHandler::construct_writer(int client_socket, size_t need_write,
    ...
    auto result = [=]() mutable {
        ...
        size_t want_write = std::min(src_size - bytes_written_from_src, need_write - bytes_written_total);
        ssize_t written = send(client_socket, src + bytes_written_from_src, want_write, MSG_NOSIGNAL);
        if (written == -1 && errno != EAGAIN) {
            throw SocketException("Ошибка при записи данных в сокет.");
        } else if (written == 0) {
            throw BadInputException("Клиент отключился.");
