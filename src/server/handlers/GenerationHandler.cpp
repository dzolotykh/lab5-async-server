    ...
    Response GenerationHandler::handle() {
        ...
        std::future<std::string> image_file = tp.add_task([this]() {
            ...
            if (num_vertex <= 0 || num_edges < 0) {
                throw Exceptions::InvalidInputException("Неправильно указано количество вершин или количество ребер.");
            }
            if (num_vertex > 200) {
                throw Exceptions::InvalidInputException("Слишком много вершин в графе. Максимальное количество вершин 200.");
            }
            ...
            for (int i = 0; i < num_edges; ++i) {
                ...
                if (used_edges.find(std::make_pair(std::min(u, v), std::max(u, v))) != used_edges.end()) {
