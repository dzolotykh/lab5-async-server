...
class NotEnoughBytesException : public SocketException {
   public:
    explicit NotEnoughBytesException(const Socket& s, size_t need_bytes, size_t available_bytes)
        : SocketException("Cannot read ") {
        message += std::to_string(need_bytes) + " bytes from socket " + std::to_string(s.get_fd()) +
                   ". Bytes available: " + std::to_string(available_bytes);
    }
};

class SocketExceptionErrno : public SocketException {
   public:
    explicit SocketExceptionErrno(const Socket& s, int errno_value)
        : SocketException("Error on socket ") {
...
class SocketCreationException : public SocketException {
   public:
    explicit SocketCreationException(int errno_value)
        : SocketException("Error while creating socket. Errno value: ") {
...
class SocketBindException : public SocketException {
   public:
    explicit SocketBindException(int errno_value)
        : SocketException("Error while binding socket. Errno value: ") {
        message += std::to_string(errno_value);
    }
};
...
class SocketListenException : public SocketException {
   public:
    explicit SocketListenException(int errno_value)
        : SocketException("Error while listening socket. Errno value: ") {
        message += std::to_string(errno_value);
    }
};
...
class SocketAcceptException : public SocketException {
   public:
    explicit SocketAcceptException(int errno_value)
        : SocketException("Error while accepting socket. Errno value: ") {
        message += std::to_string(errno_value);
    }
};
